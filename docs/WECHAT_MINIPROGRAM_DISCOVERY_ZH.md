...
- 列表标题：`linux-board`
- 详情字段：`Local Name: yundrone-ytcwln`

小程序不应该把列表标题当作业务身份。列表标题通常来自系统缓存、BlueZ adapter 的 `Name/Alias`，或连接后的 GATT 设备名。它可能显示为主机名，例如 `linux-board`。

我们真正设计并控制的是广播数据里的 Local Name，也就是 `yundrone-<base36_6>`。小程序发现设备时必须以这个字段为准。
...
这是正常的。列表按 RSSI 排序，并显示完整 `localName`。当前命名里的 6 位 base36 稳定后缀用于区分设备，例如 `yundrone-ytcwln`。
