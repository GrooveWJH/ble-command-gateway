...
4. 点击“扫描 Wi-Fi”，等待 10 到 30 秒。
5. 在扫描结果里选择目标 SSID，或直接手动输入隐藏网络 SSID。
6. 输入 Wi-Fi 密码；开放网络可以留空。密码只用于本次下发，不保存到 `localStorage`。
7. 点击“确认并下发配网”，确认设备名和 SSID 后等待结果。
8. 成功时页面会显示设备返回的 SSID 和 IP；失败时页面会提示检查密码、频段兼容性和信号。

页面一次只执行一个业务命令，避免 BLE 弱链路下并发命令互相干扰。

界面按小白用户的旅程组织：顶部显示浏览器支持、设备名、当前命令和协议版本；左侧步骤栏说明当前在“环境检查 / 连接设备 / 扫描 Wi-Fi / 选择网络 / 输入密码 / 下发配网 / 结果确认”的哪一步；中间用高密度表格展示 Wi-Fi 扫描结果；右侧固定显示 SSID、密码、确认摘要和恢复建议。

## 调试模式

Debug 面板对应 CLI verbose 的人类可读版本：
...
调试面板支持过滤、暂停、清空和导出 `.trace` 文本，便于用户把失败现场发给开发者。
